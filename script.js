...
let selectedSeat = 0;
let discountPriceContainer = document.getElementById(
...
for (let index = 0; index < seats.length; index++) {
  let seat = seats[index];
    ...
    seat.addEventListener("click", function ABC() {
      if(!(seat.classList.contains('selected')) && selectedSeat === 4) {
        alert('you can not selected more then 4 seat')
      }
    });
  ...
  seat.addEventListener("click", function seatCalculation(e) {
    if (selectedSeat == 4) {
      ...
      SelectedSeatContainer.innerHTML += `
            ...
            <div class="w-4/12"><span class="text-[#03071299] ">Economy </span></div>
